...
public class LocalVersion extends Application {
				...
				for (int j=0; j<10; j++) {
					...
					if (!guiBoardObject.masterBoard.board[i][j].checkIfEmpty()) {
						...
						// If blocked, place fire
							...
							//redCircle.setFill(Color.rgb(198,60,81));
	...
	public void start(Stage primaryStage) {
		...
		endText.setFont(Font.font("Comic Sans MS", FontWeight.BOLD, 20));
		...
		startButton.setOnAction(e -> primaryStage.setScene(boardScene));
		restartButton.setOnAction(e -> new LocalVersion());
		winButton.setOnAction(e -> primaryStage.setScene(endScene));
	...
	class squareClickHandler implements EventHandler<MouseEvent> {
		...
		public void handle(MouseEvent event) {
			...
			guiBoardObject.masterBoard.feedCoord(coord[1], coord[0]);
			guiBoardObject.refreshBoard();
		}
	}
	...
	class circleClickHandler implements EventHandler<MouseEvent> {
	 ...
	 public static class Board {
		 ...
		 public boolean feedCoord(int guiX, int guiY) {
			 switch (turnPhase) {
			 case 0:
		 ...
		 public void winner(pieceColor color) {
			 endText.setText("Congratulations " + color + ", you won!");
			 winButton.fire();
